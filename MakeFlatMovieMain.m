...
// Usage: makeflatmovie FLATMOVIE ARCHIVE

int main (int argc, const char * argv[]) {
    NSAutoreleasePool * pool = [[NSAutoreleasePool alloc] init];
	crcs = NULL;

  // Open an archive file and convert movie frames to a .flat movie file
  
  if (argc != 3) {
    fprintf(stderr, "makeflatmovie FLATMOVIE ARCHIVE\n");
    exit(1);
  }
  
  const char *flatmovieOutputFilenameStr = argv[1];
  const char *archiveInputFilenameStr = argv[2];
  NSString *flatmovieOutputFilename = [NSString stringWithFormat:@"%s", flatmovieOutputFilenameStr];
  NSString *archiveInputFilename = [NSString stringWithFormat:@"%s", archiveInputFilenameStr];
  
	EasyArchive *archive = [[EasyArchive alloc] initWithFilename:archiveInputFilename];
  
	BOOL worked = [archive openForReading];
	assert(worked);
	...
	worked = [FlatMovieFile flattenMovie:mArchive flatMoviePath:flatmovieOutputFilename];
	assert(worked);
	...
	worked = [flatFile validateFlattenedMovie:mArchive flatMoviePath:flatmovieOutputFilename];
	assert(worked);
